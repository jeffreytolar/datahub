...
| <div className="path-line"><span className="path-main">temp_table_patterns</span></div> <div className="type-name-line"><span className="type-name">array</span></div> | Regex patterns for temporary tables to filter in lineage ingestion. Patterns match from the start of the table name only, not the entire name - anchor with '^...$' for an exact match. This is useful for platforms like Athena that don't have native temp tables but use naming patterns for fake temp tables. <div className="default-line default-line-with-docs">Default: <span className="default-value">&#91;&#93;</span></div> |
...
```javascript
{
  ...
  "properties": {
    ...
    "usage": {
      ...
      "default": {
        "bucket_duration": "DAY",
        "end_time": "2026-08-17T20:37:47.427836Z",
    ...
    "temp_table_patterns": {
      "default": [],
      "description": "Regex patterns for temporary tables to filter in lineage ingestion. Patterns match from the start of the table name only, not the entire name - anchor with '^...$' for an exact match. This is useful for platforms like Athena that don't have native temp tables but use naming patterns for fake temp tables.",
      "items": {
        "type": "string"
      },
